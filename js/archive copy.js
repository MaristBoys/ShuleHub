...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // *** SOLUZIONE ALL'ERRORE: Inizializza DataTables QUI, PRIMA DEI CONTROLLI DI LOGIN E PROFILO ***
    ...
    if (!isLoggedIn || !userData || !userData.email) {
        ...
        documentsDataTable.clear().draw(); // Ora è sicuro chiamare clear()
        setTimeout(() => {
...
async function loadArchivedFiles(requestedAuthorFilter = '') {
    ...
    if (!shouldFetch) {
        documentsDataTable.clear().draw(); // Questa chiamata era già sicura perché all'interno di una funzione.
        return;
    }
