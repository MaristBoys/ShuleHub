...
// mainLogoutButton rimosso // const mainLogoutButton = document.getElementById('main-logout-button');
...
function initializeNavbarListeners() {
    ...
    // Rimosso listener per mainLogoutButton // if (mainLogoutButton) mainLogoutButton.addEventListener('click', logout);
...
// Aggiunto un parametro 'clearResult' per controllare la pulizia del resultDiv
function updateUIForLoginState(isLoggedIn, userData = null, clearResult = true) {
    ...
    if (isLoggedIn) {
        ...
        // mainLogoutButton rimosso da qui // mainLogoutButton.style.display = 'none';
...
async function logout() {
    try {
        ...
        localStorage.removeItem('isLoggedIn');
        localStorage.removeItem('userData');
        resultDiv.innerHTML = ''; // Puliamo il resultDiv al logout
...
function simulateLogin() {
    ...
    const mockUserData = {
        ...
        googlePicture: "https://lh3.googleusercontent.com/a/ACg8ocKw_NnQZ2Q_C9d2n9Z_q_x_y_z_a_b_c_d_e=s96-c"
    };
    localStorage.setItem('isLoggedIn', 'true');
